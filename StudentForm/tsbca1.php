...
<?php
    ...
    if ($result->num_rows > 0)
    {
        ...
        $str='<form class="w3-container" name="tsbca-i" action="validateForm.php" onsubmit="return validateForm()" method="post" style="font-size: 17px;margin-left:5%;">';
        $str.='<input type="hidden" name="sem" value="5"/>';
        
        
    //TotalAvailableCredits    
        
        $sql="SELECT bcasubjectlist.credits FROM bcasubjectlist WHERE bcasubjectlist.course_code='tsbca1L' AND course_name='min_Sem5'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
        ...
        $sql="SELECT bcasubjectlist.credits FROM bcasubjectlist WHERE bcasubjectlist.course_code='tsbca1H' AND course_name='max_Sem5'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
