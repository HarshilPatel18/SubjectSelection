...
<?php
    ...
    if ($result->num_rows > 0)
    {
        ...
        $str='<form class="w3-container" name="fsbca-ii" action="validateForm.php" onsubmit="return validateForm()" method="post" style="font-size: 17px;margin-left:5%;">';
        $str.='<input type="hidden" name="sem" value="2"/>';
        
        
    //TotalAvailableCredits    
        
        $sql="SELECT bcasubjectlist.credits FROM bcasubjectlist WHERE bcasubjectlist.course_code='fsbca2L' AND course_name='min_Sem2'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
        ...
        $sql="SELECT bcasubjectlist.credits FROM bcasubjectlist WHERE bcasubjectlist.course_code='fsbca2H' AND course_name='max_Sem2'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
